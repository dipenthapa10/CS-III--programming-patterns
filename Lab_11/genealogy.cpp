...
// concrete visitor
class MiddlenameUpdater : public PersonVisitor
{
   void visit(Man *m) override
   {
      ...
      if (father != nullptr)
      {
         string newName = m->getFirstName() + " " + father->getFirstName();
         m->setFirstName(newName);
      }
   }

   void visit(Woman *m) override
   {
      Person *mother = m->getMother();
      if (mother != nullptr)
      {
         string momFirst = mother->getFirstName().substr(0, mother->getFirstName().find(' '));

         string newName = m->getFirstName() + " " + momFirst;
         m->setFirstName(newName);
      }
   }
};
...
int main()
{
   ...
   MiddlenameUpdater *mu = new MiddlenameUpdater;
